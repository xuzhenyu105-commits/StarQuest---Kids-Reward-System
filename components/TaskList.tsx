...
const TaskList: React.FC<TaskListProps> = ({ tasks, onCompleteTask, onAddTask, onDeleteTask }) => {
  ...
  const handleManualAdd = () => {
    ...
    const task: Task = {
      ...
      completionTime: newTaskTime || undefined
    };
    onAddTask(task);
    setNewTaskTitle('');
    setNewTaskTime('');
        ...
        {!showAddForm ? (
          ...
          <div className="space-y-4 animate-in fade-in slide-in-from-top-4 duration-300">
            ...
            <div className="flex flex-col gap-3 px-1">
              ...
              <div className="flex items-center justify-between">
                 <span className="text-xs font-black text-gray-400 uppercase tracking-widest">完成时间:</span>
                 <input 
                   type="time" 
                   value={newTaskTime}
                   onChange={(e) => setNewTaskTime(e.target.value)}
                   className="bg-gray-100 px-3 py-1.5 rounded-xl text-xs font-bold text-indigo-600 outline-none focus:bg-white border-2 border-transparent focus:border-indigo-100 transition-all"
                 />
              </div>
          ...
          return (
            ...
            >
              <div className="flex items-center gap-4 min-w-0 flex-1">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-2 flex-wrap">
                    ...
                    {task.completionTime && (
                      <span className="text-[9px] font-black uppercase tracking-tighter px-2 py-0.5 rounded-full bg-slate-100 text-slate-500 flex items-center gap-1">
                        <Clock size={10} /> {task.completionTime}
                      </span>
                    )}
              ...
              <button 
                ...
                className="opacity-0 group-hover:opacity-100 text-gray-200 hover:text-rose-500 transition-all p-2 rounded-xl hover:bg-rose-50 flex-shrink-0"
              >
                <Trash2 size={20} />
              </button>
            </div>
